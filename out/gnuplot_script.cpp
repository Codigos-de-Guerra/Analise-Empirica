...
int main() {
    ...
    if(s[0] == '1'){
        system("\'Tempo(ms)_ILS.txt\' using 1:2 title \"Iterative Linear Search\"");
    }
    if(s[1] == '1'){
        if(!flag) system(",");
        system("\'Tempo(ms)_IBS.txt\' using 1:3 title \"Iterative Binary Search\"");
    }
    if(s[2] == '1'){
        if(!flag) system(",");
        system("\'Tempo(ms)_RBS.txt\' using 1:4 title \"Recursive Binary Search\"");
    }
    if(s[3] == '1'){
        if(!flag) system(",");
        system("\'Tempo(ms)_ITS.txt\' using 1:5 title \"Iterative Ternary Search\"");
    }
    if(s[4] == '1'){
        if(!flag) system(",");
        system("\'Tempo(ms)_RTS.txt\' using 1:6 title \"Recursive Ternary Search\"");
    }
    if(s[5] == '1'){
        if(!flag) system(",");
        system("\'Tempo(ms)_FBS.txt\' using 1:7 title \"Fibonacci Search\"");
    }
    if(s[6] == '1'){
        if(!flag) system(",");
        system("\'Tempo(ms)_JS.txt\' using 1:8 title \"Jump Search\"");
    }
